...
If installation fails due to compilation issues, you may need to:
1. Install older Cython: pip install "Cython<3.0"
2. Install compatible NumPy: pip install "numpy<2.0"
3. Then install cochlea: pip install git+https://github.com/mrkrd/cochlea.git
...
def anm(stim, fs_in, stim_pres_db, parallel=True, n_jobs=4,  # Reduced from -1 to 4 cores
...
def main():
    ...
    use_parallel = True  # Set to False for memory-efficient processing
    max_parallel_jobs = 2  # Reduce from 4 to 2 for even less memory usage
